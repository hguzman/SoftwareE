...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { 
	...
	$q = "SELECT COUNT(idEmail) FROM email WHERE idPersona=".$idPersona;
...
//echo $records;
$q = "SELECT * FROM email WHERE idPersona=".$idPersona." ORDER BY idEmail ASC LIMIT $start, $display";	
...
echo '<p align="center"><table cellspacing="8" cellpadding="8" class="tabla">';
	...
	echo '<tr>
	<td><img src="images/adduser.png" width="38" heigth="38"/></td><td><a href="addEmail.php?idPersona='.$idPersona.'">Agregar E-mail</a></td>
</tr>';
...
while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
	$bg = ($bg=='<tr class="alt" >' ? '<tr>' : '<tr class="alt" >');
		...
		echo '<td align="left"><a href="editEmail.php?idEmail=' . $row['idEmail'] . '"><img src="images/preferences-composer.png" width="32" height="32" /></a></td>';
	
		echo '<td align="left"><a href="delEmail.php?idEmail=' . $row['idEmail'] . '&idPersona='.$_GET['idPersona'].'" onclick="return confirmSubmit()"><img src="images/error.png" width="32" height="32" /></a></td>';
...
mysqli_close($dbc);
		...
		if ($pages > 1) {
			...
			if ($current_page != 1) {
				echo '<a href="verEmail.php?s=' . ($start - $display) . '&p=' . $pages . '&sort=' . $sort . '">Anterior</a> ';
			}
			
			
			for ($i = 1; $i <= $pages; $i++) {
				if ($i != $current_page) {
					echo '<a href="verEmail.php?s=' . (($display * ($i - 1))) . '&p=' . $pages . '&sort=' . $sort . '">' . $i . '</a> ';
				} else {
					echo $i . ' ';
				}
			} 
			if ($current_page != $pages) {
				echo '<a href="verEmail.php?s=' . ($start + $display) . '&p=' . $pages . '&sort=' . $sort . '">Siguiente</a>';
			}
