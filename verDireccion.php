...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { 
	...
	$q = "SELECT COUNT(idDireccion) FROM direccion WHERE idPersona=".$idPersona;
...
//echo $records;
$q = "SELECT * FROM direccion WHERE idPersona=".$idPersona." ORDER BY idDireccion ASC LIMIT $start, $display";	
...
echo '<p align="center"><table cellspacing="8" cellpadding="8" class="tabla">';
	...
	echo '<tr>
	<td><img src="images/adduser.png" width="38" heigth="38"/></td><td><a href="addDireccion.php?idPersona='.$idPersona.'">Agregar direccion</a></td>
</tr>';
...
<tr>
	...
	<th align="left"><b>Direccion</b></th>
...
while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
	$bg = ($bg=='<tr class="alt" >' ? '<tr>' : '<tr class="alt" >');
		...
		echo '<td align="left"><a href="editDireccion.php?idDireccion=' . $row['idDireccion'] . '"><img src="images/preferences-composer.png" width="32" height="32" /></a></td>';
	
		echo '<td align="left"><a href="delDireccion.php?idDireccion=' . $row['idDireccion'] . '&idPersona='.$_GET['idPersona'].'" onclick="return confirmSubmit()"><img src="images/error.png" width="32" height="32" /></a></td>';
...
mysqli_close($dbc);
		...
		if ($pages > 1) {
			...
			if ($current_page != 1) {
				echo '<a href="verDireccion.php?s=' . ($start - $display) . '&p=' . $pages . '&sort=' . $sort . '">Anterior</a> ';
			}
			
			
			for ($i = 1; $i <= $pages; $i++) {
				if ($i != $current_page) {
					echo '<a href="verDireccion.php?s=' . (($display * ($i - 1))) . '&p=' . $pages . '&sort=' . $sort . '">' . $i . '</a> ';
				} else {
					echo $i . ' ';
				}
			} 
			if ($current_page != $pages) {
				echo '<a href="verDireccion.php?s=' . ($start + $display) . '&p=' . $pages . '&sort=' . $sort . '">Siguiente</a>';
			}
